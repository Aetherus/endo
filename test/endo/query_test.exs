defmodule Endo.QueryTest do
  ...
  describe "select/3" do
    ...
    test "binary operator with aggregation on one side and non-aggregation non-literal expression on the other" do
      assert_compile_time_raise(
        ArgumentError,
        "Can't mix aggregation and non-aggregation in one formula.",
        fn ->
          import Endo.Query
    ...
    test "free-style non-aggregation" do
      ...
      query =
        from("table1") |> select([p], non_agg("my_func(?, ?)", p["foo"], p["bar"] <> ^suffix))
  ...
  describe "where/3" do
    ...
    test "aggregation" do
      assert_compile_time_raise(
        ArgumentError,
        "Where condition must not contain aggregation.",
        fn ->
          import Endo.Query
  ...
  describe "order_by/3" do
    test "non-aggregation" do
      ...
      query =
        from("table1")
        |> order_by([q], [q["foo"], asc: q["bar"], desc: q["baz"], asc: q[^f] + q["xxx"]])
  ...
  describe "having/3" do
    ...
    test "non-aggregate" do
      assert_compile_time_raise(
        ArgumentError,
        "Only an aggregation expression is allowed in having clause.",
        fn ->
          import Endo.Query
